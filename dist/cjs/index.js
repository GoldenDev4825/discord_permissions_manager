...
exports.DiscordPermissionsManager = void 0;
...
class DiscordPermissionsManager {
    ...
    canUserRunCommand(userId, command, userPermissions, guildOwnerId) {
        if (userId === guildOwnerId)
            return true;
        if ((userPermissions & v10_1.PermissionFlagsBits.Administrator) !== 0n)
            return true;
    ...
    calculateChannelPermissions(member, channel, roles, guildOwnerId) {
        if (member.user?.id === guildOwnerId) {
            return BigInt(v10_1.PermissionFlagsBits.Administrator);
        }
        const memberRoles = roles.filter(r => member.roles.includes(r.id));
        ...
        if ((permissions & v10_1.PermissionFlagsBits.Administrator) !== 0n) {
            return BigInt(v10_1.PermissionFlagsBits.Administrator);
        }
    ...
    hasChannelManageAccess(permissions) {
        return (permissions & v10_1.PermissionFlagsBits.ManageChannels) !== 0n ||
            (permissions & v10_1.PermissionFlagsBits.Administrator) !== 0n;
    }
    getManageableRoles(member, roles, guildOwnerId) {
        ...
        const highestEligibleRole = memberRoles
            ...
            return (perms & v10_1.PermissionFlagsBits.ManageRoles) !== 0n ||
                (perms & v10_1.PermissionFlagsBits.Administrator) !== 0n;
        })
    ...
    async validateCommandPermissionAccess(commandId) {
        ...
        if ((permissions & v10_1.PermissionFlagsBits.Administrator) !== 0n)
            return;
        const hasManageGuild = (permissions & v10_1.PermissionFlagsBits.ManageGuild) !== 0n;
        const hasManageRoles = (permissions & v10_1.PermissionFlagsBits.ManageRoles) !== 0n;
    ...
    async getAvailableManageableResources() {
        ...
        const hasAdministrator = (permissions & v10_1.PermissionFlagsBits.Administrator) !== 0n;
        const hasManageChannels = hasAdministrator || ((permissions & v10_1.PermissionFlagsBits.ManageChannels) !== 0n);
        const guild = await this.fetchGuild();
...
exports.DiscordPermissionsManager = DiscordPermissionsManager;
